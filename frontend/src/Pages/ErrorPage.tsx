  ...
  useEffect(() => {
    ...
    function draw() {
      ...
      bordersCoords.forEach((coords) => {
        if (
          x + dx > coords.x + coords.width - ballRadius ||
          x + dx < coords.x
        ) {
          dx = -dx;
        }
        if (
          y + dy > coords.y + coords.height - ballRadius ||
          y + dy < coords.y
        ) {
          dy = -dy;
        }
        ...
        if (
          x2 + dx2 > coords.x + coords.width - ballRadius ||
          x2 + dx2 < coords.x
        ) {
          dx2 = -dx2;
        }
        if (
          y2 + dy2 > coords.y + coords.height - ballRadius ||
          y2 + dy2 < coords.y
        ) {
          dy2 = -dy2;
        }
    ...
    <>
      <PageLayout>
        ...
        <div className="flex justify-center items-center">
          <main className="min-w-fit w-72 flex flex-col border border-white rounded-md p-20 gap-4">
