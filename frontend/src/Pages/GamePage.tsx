...
interface GameInvitationResponse { // move to models
	message: string;
	sender: string;
	receiver: string;
}
...
const GamePage = () => {
	...
	socket?.on("gameInvitationResponse", (response: GameInvitationResponse) => {
		...
		if (response.message === "valider" && response.sender === game?.name_p1) {
			setShowEditModal(false);
		} else if (response.message === "refuser" && response.sender === game?.name_p1) {
