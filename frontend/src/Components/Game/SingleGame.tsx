...
interface GameProps {
  game: IGame;
  onFinish: (updatedGame: GameUpdate) => void;
}
...
const Game = ({ game, onFinish }: GameProps) => {
  ...
  useEffect(() => {
    ...
    const maxSpeed = 2 * (game?.difficulty || 1);
    const acceleration = 0.1 * (game?.difficulty || 1);
    const moveSpeed = 3 * (game?.difficulty || 1);

    if (context && canvas) {
      player2PaddleRef.current.x = canvas.width - 100 - 10;
      ballRef.current.x = canvas.width / 2;
      ballRef.current.y = canvas.height / 2;
      ballRef.current.speed = 1 + (game?.difficulty || 1);
      ...
      const endGame = () => {
        ...
        const updateGame = {
          gameId: game.id,
          scorePlayer1: player1PaddleRef.current.score,
          scorePlayer2: player2PaddleRef.current.score,
          winner:
      ...
      const checkVictoryCondition = () => {
        if (game.gameType === "time" && timer.current >= (game.value || 0)) {
          clearInterval(interval);
          endGame();
        } else if (
          game.gameType === "1972" &&
          ...
          game.gameType === "points" &&
          (player1PaddleRef.current.score >= (game.value || 1) ||
            player2PaddleRef.current.score >= (game.value || 1))
        ) {
          clearInterval(interval);
          endGame();
        }
      ...
      const aiPlayer = () => {
        if (!game.player2 && !gameFinished) {
          ...
          const aiSpeed = 2 + (game.difficulty || 1);
