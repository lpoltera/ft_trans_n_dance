...
const Game = ({ game, onFinish }: GameProps) => {
	...
	// let powerUp1 = false;
	// const [powerUp1, setPowerUp1] = useState(false);
	// const [powerUp2, setPowerUp2] = useState(false);
	...
	// const pwrTimer2 = useRef(0);
	// const pwrTimer1 = useRef(0);
	// const setPwrTimer1 = (value: number) => {
	//   pwrTimer1.current = value;
	// };
	// const setPwrTimer2 = (value: number) => {
	//   pwrTimer2.current = value;
	// };
	...
	useEffect(() => {
		...
		if (context && canvas) {
			...
			const resetBall = (playerScored: number) => {
				if (game.power_ups) setPowerUpActive(true);
	...
	const drawPowerUp = useCallback(
		(context: CanvasRenderingContext2D, width: number, height: number) => {
			...
			if (powerUp2.current) {
				...
				// context.beginPath();
				// context.arc(width / 2, (height / 5) * 2, 20, 0, Math.PI * 2, true);
				// context.closePath();
				// context.fill();
			}
			if (powerUp1.current) {
				...
				// context.beginPath();
				// context.arc(width / 2, (height / 5) * 3, 20, 0, Math.PI * 2, true);
				// context.closePath();
				// context.fill();
			}
			// }
		},
		[powerUpActive.current, powerUp1, powerUp2]
	);
